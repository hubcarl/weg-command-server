...
exports.run = function (opt) {
    step(
        ...
        function checkServerScript() {
            ...
            var script = path.join(root, opt.entry || fis.get('server.entry') || 'server.js');

            //fis.log.info('>>>start run script:' + script);
        ...
        function startServerScript() {
            var script = path.join(opt.root, opt.entry || fis.get('server.entry') || 'server.js');
            ...
            opt.script = script;
            opt.serverDir = path.join(opt.root, fis.get('project.serverDir'));
            ...
            var onData = function (chunk) {
                ...
                if (chunk.indexOf('The server is runing.') == -1) {
                    console.log(chunk);
                }
                ...
                if (~chunk.indexOf('Error')) {
                    ...
                    setTimeout(function () {
                        _.open('http://127.0.0.1' + (opt.port == 80 ? '/' : ':' + opt.port + '/'), function () {
                            fis.log.notice('http://127.0.0.1' + (opt.port == 80 ? '/' : ':' + opt.port ) + ' Or browse ' + ('http://' + _.hostname + (opt.port == 80 ? '/' : ':' + opt.port + '/')).yellow.bold + '\n');
                            //process.exit();
                        });
                    }, 200);
                }
